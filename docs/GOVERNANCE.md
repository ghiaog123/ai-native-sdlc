...
1. **`PreToolUse` hooks provide enforcement.** `artifact-gate.sh` blocks code writes when the active change has no `plan.md`, and `prod-guard.sh` blocks production commands. These controls exist to make hard boundaries fail closed at the moment of action. They run only at the tool boundary and add no per-message prompt cost.
